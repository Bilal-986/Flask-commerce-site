...
def register():
    ...
    form = RegisterForm()
    if form.validate_on_submit():
        # Protect with invite code
        required_code = os.environ.get('ADMIN_INVITE_CODE')
        provided_code = request.form.get('invite_code')
        if required_code and provided_code != required_code:
            flash('Invalid or missing admin invite code.', 'error')
