...
# Use safe defaults if env vars are missing to ease local development
secret_key = os.environ.get("SECRET_KEY") or os.urandom(24).hex()
app.config["SECRET_KEY"] = secret_key

# Default to a local SQLite database if DB_URI is not provided
default_sqlite_path = os.path.join(os.path.dirname(__file__), 'test.db')
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get("DB_URI") or f"sqlite:///{default_sqlite_path}"
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# Mail configuration with empty defaults (features depending on mail may not work without creds)
...
# Stripe key optional for local dev; warn if missing
stripe_private_key = os.environ.get("STRIPE_PRIVATE")
if stripe_private_key:
    stripe.api_key = stripe_private_key
else:
    print("Warning: STRIPE_PRIVATE not set; payment features will be disabled.")
...
def home():
	...
	items = query.all()
	# Distinct categories for filter menu
	categories = sorted({i.category for i in Item.query.all()})
	return render_template("home.html", items=items, categories=categories, selected_category=selected_category, sort=sort)
...
def search():
	...
	sort = request.args.get('sort')
	search = "%{}%".format(query)
	q = Item.query.filter(Item.name.like(search))
	if selected_category and selected_category != 'all':
	...
	items = q.all()
	categories = sorted({i.category for i in Item.query.all()})
	return render_template('home.html', items=items, search=True, query=query, categories=categories, selected_category=selected_category, sort=sort)
